...
### 为什么选择 Miniforge ?
> **重要协议说明：** 由于 Anaconda 商业协议的变更，本仓库已全面放弃 Anaconda/Miniconda 发行版，转而使用 **[Miniforge](https://github.com/conda-forge/miniforge)** 。
- **Miniforge 的优势:** 默认使用 `conda-forge` 频道，完全避开商业授权限制，且对 ARM 架构支持更佳。
- **Channel 策略:** 强制移除 `defaults` 频道，确保所有依赖项均来自开源社区维护的 `conda-forge`。
...
| **conda (Miniforge)** | **Legacy** | 仅为科研任务保留兼容性，**不再主动更新**。 |
...
## 如何使用 (Legacy: Miniforge)

由于 Conda 在科学计算领域积淀深厚，部分旧项目仍保留 .yaml 配置。注意：请确保你使用的是 **[Miniforge](https://github.com/conda-forge/miniforge)** 。

### 禁用 defaults 频道 (安全性配置)

为了避免 Anaconda 协议限制并拥抱开源社区，在全局配置中禁用 `defaults`:

```shell
# 禁用 defaults 频道并设置 strict 优先级
...
仓库内的所有 `environment.yaml` 均已更新，强制指定 `conda-forge` 并禁用 `defaults` 频道。

```yaml
# 示例配置片段
channels:
  - conda-forge
  - nodefaults  # 强制禁止使用 defaults
dependencies:
  ...
```
...
- [x] 迁移至 Miniforge，不再使用 Anaconda/Miniconda 以及 defaults 频道。
- [x] 引入 uv 替换原生 pip 。
